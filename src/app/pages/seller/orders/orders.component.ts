...
@Component({
  ...
  imports: [
              ...
              NzIconModule, NzRateModule, NzSelectModule,
              MatTooltipModule
           ],
...
export class OrdersComponent implements OnInit {
  ...
  getOrders(pageIndex: number = 1, pageSize: number = 10, col: string = 'update', order: boolean = false) {
    this.loading = true;
    this.sellerService.getOrders(pageIndex, pageSize, col, order)?.then(response => {
      this.loading = false;
  ...
  updateOrders(id:string, event: any, col: string): void {
    ...
    this.sellerService.editOrder(id, params)?.then(response => {
   ...
   */
  getProducts() {
    this.sellerService.getAllProducts()?.then(response => {
