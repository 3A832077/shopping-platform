...
export class LoginComponent implements OnInit {
  ...
  async login() {
    if (this.form.valid) {
      ...
      const result = await this.sellerService.login(email, password);
      if (result?.error) {
  ...
  async register() {
    if (this.form.valid) {
      const { email, password} = this.form.value;
      const result = await this.sellerService.register(email, password);
      if (result?.error) {
        console.error(result.error);
      }
      else {
  ...
  async sendResetEmail() {
    if (this.email) {
      const result = await this.sellerService.forgetPassword(this.email);
      if (result?.error) {
