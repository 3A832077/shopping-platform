...
export class CheckoutComponent implements OnInit {
  ...
  getSelectedStoreIndex(): number {
    return this.storeList.findIndex(
      (store) => store.id === this.form.value.store
    );
  }
        ...
        this.supabaseService
          ...
          ?.then(({ data, error }) => {
            if (error) {
              ...
              // 用 forkJoin 等所有刪除完成
              const removeObservables = this.cartItems.map(
                (item) =>
              ...
              const updateObservables = this.cartItems.map(
                (item) =>
                  this.supabaseService.updateProductStock(
                    item.products.id,
                    item.quantity
                  ) || of(null)
              );
